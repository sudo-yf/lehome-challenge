...
ensure_isaacsim_symlink_ready() {
  ...
  python -c "import isaacsim" >/dev/null 2>&1 || {
    style_err "无法导入 isaacsim；请先确认 uv sync 已完成，必要时补装系统图形库后重试。"
    return 1
  }
  local isaacsim_path
  isaacsim_path="$(resolve_isaacsim_package_path)"
  [[ -n "$isaacsim_path" ]] || { style_err "无法解析 isaacsim 安装路径"; return 1; }
  style_dim "检测到 IsaacSim 包路径: $isaacsim_path"
  safe_symlink "$isaacsim_path" "$isaaclab_dir/_isaac_sim"
}
...
run_guided_flow_wizard() {
  ...
  local want_ai_tools=0 want_git_basic=0 want_shell=0 want_xvla_check=0
  local line item_id
  while IFS= read -r line; do
    [[ -z "$line" ]] && continue
    item_id="${line%% | *}"
...
step_write_shell_shortcuts() {
  ...
  style_dim "提示: 请手动执行一次 source ~/.bashrc"
...
step_eval() {
  ...
  kv "Dataset root" "$dataset_root"; kv "Log file" "logs/$log_name"; kv "XVFB" "enabled"
...
PYEOF
  ...
  local cmd log_file
  cmd="$python_bin $train_bridge --config-path=$gen_config --headless"
  ...
  if [[ "$run_now" =~ ^[Yy] ]]; then
    ...
    (
      cd "$ROOT_DIR"
      nohup "$python_bin" "$train_bridge" --config-path="$gen_config" --headless >"$log_file" 2>&1 &
      echo "训练已启动，PID: $!"
