...
cmd=(python "$BUNDLE_ROOT/scripts/wandb_sweep.py" --model "$MODEL" --config-file "$(resolve_bundle_path "$SWEEP_CONFIG")" --count 1 --steps 100 --disable-artifact)
if [[ -n "$TRAIN_CONFIG" ]]; then
  cmd+=(--train-config "$(resolve_bundle_path "$TRAIN_CONFIG")")
fi
cmd+=("$@")
section "执行 100 步 smoke"
kv "Mode" "$SMOKE_MODE"
cmd_preview "${cmd[*]}"
exec "${cmd[@]}"
