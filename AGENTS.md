...
- 禁止以"最小可用"、"先跑通再说"、"临时兜底"替代官方完整流程；涉及安装、训练、评估、WandB、XVLA 等任务时，必须优先按官方文档与仓库教材的完整做法执行，不得擅自做功能裁剪、接口降级或验证缩水。
- 若官方文档、仓库文档与现有脚本存在差异，必须先逐项报告差异（文件 + 行号 + 影响），给出对齐官方流程的方案，并在获得人工确认后再修改。
...
3. 给出与官方教材/文档对齐的完整方案，禁止以"最小可用"作为默认目标（先解释再执行）。
